...
// Extend the User type to include optional address and other profile fields if needed
type User = {
  _id: string;
  fullName: string;
  email: string;
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // For demonstration, we'll extend the auth user data with mock profile info
      const mockProfileData: User = {
        ...
        district: 'Gasabo',
        zipCode:'90210',
        bio: 'Insurance professional with expertise in multiple policy types.',
        profilePicture:  '',
        licenseNumber: (authUser.role === 'AGENT' ? 'INS-12345-AG' : ''),
        licenseExpiry:  (authUser.role === 'AGENT' ? '2025-12-31' : ''),
      };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      try {
        ...
        // For this demo, we'll just update the auth context
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/users/${profile._id}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('ezinsure_token')}`
          },
        ...
        localStorage.setItem('ezinsure_user', JSON.stringify(updatedUser.data));
                ...
                <div className="flex flex-col items-center w-full md:w-1/3">
                  ...
                  {profile.agentCode && (
                    <div className="mt-4 w-full">
                      <div className="text-center py-2 px-4 rounded-full bg-gray-100 text-gray-800">
                  ...
                  {profile.commissionRate && (
                    <div className="mt-4 w-full">
                      <div className="text-center py-2 px-4 rounded-full bg-green-100 text-green-800">
