...
export type LivestockApplicationViewRole =
  | 'vet'
  | 'admin'
  | 'super_admin'
  | 'finance'
...
/** SONARWA representative decision stored on the application after review. */
export type SonarwaReviewDecision = 'APPROVED' | 'APPROVED_WITH_CHANGES' | 'REJECTED';

export interface SonarwaReviewRecord {
  decision: SonarwaReviewDecision;
  reviewedAt: string;
  reviewedByUserId?: string;
  reviewedByName?: string;
  /** Required when decision is APPROVED_WITH_CHANGES. */
  changeComment?: string;
  /** Corrected evidence document uploaded by SONARWA (excl. fraud / errors). */
  correctionDocumentUrl?: string;
  /** Vet commission before SONARWA adjustment. */
  originalVeterinaryCommission?: number;
  /** Vet commission SONARWA confirmed should be paid. */
